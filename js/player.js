  ...
  constructor() {
    this.songs = [
      ...
      },
      {
        ...
        title: '寓言 - 希林娜依高',
        singer: '寓言 - 希林娜依高',
        songUrl: './songs/02.mp3',
        imageUrl: './images/songs/02.jpg',
      },
    ]
  }
  //根据索引获取歌曲的方法
...
class PlayerCreator {
  ...
  songTime() {
    let totalMinute =
      parseInt(this.audio.duration / 60) < 10
        ? '0' + parseInt(this.audio.duration / 60)
        : parseInt(this.audio.duration / 60)
    let totalSecond =
      parseInt(this.audio.duration % 60) < 10
        ? '0' + parseInt(this.audio.duration % 60)
        : parseInt(this.audio.duration % 60)
    ...
    ele.mousedown((e) => {
      ...
      $(document).mousemove((e) => {
        if (flag === true) {
          ...
          let _distance = Math.max(
            0,
            Math.min(_left, father.outerWidth(false) - ele.outerWidth(false))
          )
